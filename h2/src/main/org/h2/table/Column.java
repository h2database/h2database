...
public class Column implements HasSQL, Typed {
    ...
    private String getCreateSQL(StringBuilder builder, boolean forMeta) {
        ...
        boolean identity = !forMeta && sequence != null;
        if (identity) {
            builder.append(" GENERATED BY DEFAULT AS IDENTITY(");
            sequence.getSequenceOptionsSQL(builder).append(')');
        ...
        if (convertNullToDefault && !identity) {
    ...
    private void removeNonIdentityProperties() {
        convertNullToDefault = isGenerated = nullable = false;
