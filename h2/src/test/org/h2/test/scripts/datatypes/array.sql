...
-- Issue #4364: JOIN ON cardinality guard must short-circuit before WHERE array access
CREATE TABLE ITEM(ID INT PRIMARY KEY, TAGS INT ARRAY);
> ok

INSERT INTO ITEM VALUES (1, ARRAY[1, 2, 3]);
> update count: 1

SELECT I.ID, I.TAGS
    FROM ITEM I
    JOIN SYSTEM_RANGE(1, 1000) T ON COALESCE(CARDINALITY(I.TAGS), 0) >= T.X
    WHERE I.TAGS[T.X] > 5;
> ID TAGS
> -- ----
> rows: 0
...
SELECT I.ID
    FROM ITEM I, SYSTEM_RANGE(1, 1000) T
    WHERE COALESCE(CARDINALITY(I.TAGS), 0) >= T.X
    AND I.TAGS[T.X] = 3;
>> 1
