...
>> SELECT UNIQUE( SELECT "A", "B" FROM "PUBLIC"."TEST" /* PUBLIC.TEST.tableScan */) FROM SYSTEM_RANGE(1, 1) /* PUBLIC.RANGE_INDEX */

SELECT UNIQUE(SELECT A, B FROM TEST);
>> FALSE

EXPLAIN SELECT UNIQUE(SELECT DISTINCT A, B FROM TEST);
>> SELECT TRUE FROM SYSTEM_RANGE(1, 1) /* PUBLIC.RANGE_INDEX */
