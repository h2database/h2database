...
public class ErrorCode {
    ...
    /**
     * The error with code <code>22001</code> is thrown when trying to insert a
    ...
    /**
     * The error with code <code>23502</code> is thrown when trying to insert
    ...
    /**
     * The error with code <code>23503</code> is thrown when trying to delete or
    ...
    /**
     * The error with code <code>23505</code> is thrown when trying to insert a
    ...
    /**
     * The error with code <code>23506</code> is thrown when trying to insert or
    ...
    /**
     * The error with code <code>23507</code> is thrown when updating or
    ...
    /**
     * The error with code <code>23513</code> is thrown when a check constraint
     * is violated. Example:
     *
     * <pre>
    ...
    /**
     * The error with code <code>23514</code> is thrown when evaluation of a
     * check constraint resulted in a error.
     */
    ...
    /**
     * The error with code <code>28000</code> is thrown when there is no such
    ...
    /**
     * The error with code <code>90005</code> is thrown when trying to create a
     * trigger and using the combination of SELECT and FOR EACH ROW, which we do
     * not support.
     */
    public static final int TRIGGER_SELECT_AND_ROW_BASED_NOT_SUPPORTED = 90005;

    /**
     * The error with code <code>90006</code> is thrown when trying to get a
     * value from a sequence that has run out of numbers and does not have
     * cycling enabled.
     */
    ...
    /**
     * The error with code <code>90051</code> is thrown when trying to use a
     * scale that is > precision. Example:
     *
     * <pre>
    ...
    /**
     * The error with code <code>90009</code> is thrown when trying to create a
     * sequence with an invalid combination of attributes (min value, max value,
     * start value, etc).
     */
    public static final int SEQUENCE_ATTRIBUTES_INVALID = 90009;

    /**
     * The error with code <code>90010</code> is thrown when trying to format a
     * timestamp or number using TO_CHAR with an invalid format.
     */
    public static final int INVALID_TO_CHAR_FORMAT = 90010;

    /**
     * The error with code <code>90011</code> is thrown when trying to open a
     * connection to a database using an implicit relative path, such as
     * "jdbc:h2:test" (in which case the database file would be stored in the
     * current working directory of the application). This is not allowed
     * because it can lead to confusion where the database file is, and can
     * result in multiple databases because different working directories are
     * used. Instead, use "jdbc:h2:~/name" (relative to the current user home
     * directory), use an absolute path, set the base directory (baseDir), use
     * "jdbc:h2:./name" (explicit relative path), or set the system property
     ...
     * current user, if there are no other admin users. Example:
     *
     * <pre>
     * DROP USER SA;
     * </pre>
    ...
    /**
     * The error with code <code>90021</code> is thrown when trying to change a
     * specific database property that conflicts with other database properties.
     */
    ...
    /**
     * The error with code <code>90039</code> is thrown when trying to access a
     * CLOB or BLOB object that timed out. See the database setting LOB_TIMEOUT.
     */
     ...
     * database with an unsupported cipher algorithm. Supported is AES. Example:
     *
     * <pre>
     ...
     * timestamp using TO_DATE with an invalid format.
     */
     ...
     * server if a server is already running at the same port. It could also be
     ...
     * revoke both roles and rights at the same time. Example:
     *
     * <pre>
     ...
     * drop an index that was generated by the system because of a unique or
     * referential constraint. To find out what constraint causes the problem,
     * run:
     *
     * <pre>
     ...
     * domain that doesn't exist. Example:
     *
     * <pre>
     * DROP DOMAIN UNKNOWN;
     * </pre>
     ...
     * delete a row in a result set if the result set is not updatable. Result
     * sets are only updatable if: the statement was created with updatable
     * concurrency; all columns of the result set are from the same table; the
     * table is a data table (not a system table or view); all columns of the
     * primary key or any unique index are included; all columns of the result
     * set are columns of that table.
     */
    ...
    /**
     * The error with code <code>90138</code> is thrown when trying to open a
     * persistent database using an incorrect database name. The name of a
     * persistent database contains the path and file name prefix where the data
     * is stored. The file name part of a database name must be at least two
     * characters. Example of wrong usage:
     *
     * <pre>
    ...
    /**
     * The error with code <code>90141</code> is thrown when trying to change
     * the java object serializer while there was already data in the database.
     * The serializer of the database must be set when the database is empty.
     */
    public static final int JAVA_OBJECT_SERIALIZER_CHANGE_WITH_DATA_TABLE = 90141;

    /**
     * The error with code <code>90142</code> is thrown when trying to set zero
     * for step size.
     */
